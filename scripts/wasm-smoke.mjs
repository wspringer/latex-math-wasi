...
const wasm = readFileSync("target/wasm32-unknown-unknown/release/latex_math_wasm.wasm");
const { instance } = await WebAssembly.instantiate(wasm, {});
const { memory, latex_math_alloc, latex_math_free, latex_math_render } = instance.exports;
...
const put = (bytes) => {
  const ptr = latex_math_alloc(bytes.length);
  new Uint8Array(memory.buffer, ptr, bytes.length).set(bytes);
  return ptr;
};
const reqPtr = put(request);
const blobPtr = put(font);
const packed = latex_math_render(reqPtr, request.length, blobPtr, font.length);
const ptr = Number(packed >> 32n);
const len = Number(packed & 0xffffffffn);
const out = new Uint8Array(memory.buffer, ptr, len).slice();
latex_math_free(ptr, len);
latex_math_free(reqPtr, request.length);
latex_math_free(blobPtr, font.length);
